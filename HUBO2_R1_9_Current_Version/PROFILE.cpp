	...
	if(MotionStop == 0)
	{
		...
		else if((time>=0)&&(time<=T))
		{
			...
			*/  // end working with hand too low
			...
			float REB_Angle0 = 75.;		// right elbow bend part 2
...
//	unsigned int MotionNo;
	...
	float	result5[NumOfMotionJoint];
	float	LOCALresultMTRAng[TOTAL_MTR_NUM];
...
//	MotionNo = 44;	// user should change
  ...
  FTN_half_1_cos( 1.0f,time,    6200,1200,0,0,&result5[RSR]);
  
  // RSY to -90 degrees
  FTN_half_1_cos( 1.0f,time,    6200,1200,0,0,&result5[RSY]);
  
  // REB to 30 degrees
  FTN_half_1_cos( 1.0f,time,    6200,1200,0,0,&result5[REB]);

  // LSR from home (approximately -45 degrees) to -90 degrees
  FTN_half_1_cos( 1.0f,time,    6200,1200,0,0,&result5[LSR]);

  // LSY to -90 degrees
  FTN_half_1_cos( 1.0f,time,    6200,1200,0,0,&result5[LSY]);

  // LEB to 30 degrees
  FTN_half_1_cos( 1.0f,time,    6200,1200,0,0,&result5[LEB]);


  

  // sum of all movements beginning at different times
	res[RSR] = (float)((0.-100.)*result5[RSR]+(100.-125.)*result4[RSR]+((125.-110.)*(result3[RSR])+((110.-70.)*(result2[RSR])+70.*result1[RSR])));
	res[RSY] = (float)((0.-(-90.))*result5[RSY]+(-90.-90.)*(result3[RSY])+(90.-(-90.))*(result2[RSY])-90.*(result1[RSY]));
	res[REB] = (float)((0.-(-10.))*result5[REB]+(-10.-70.)*(result3[REB])+(70.-52.)*(result2[REB])+52.*(result1[REB]));
	res[LSR] = (float)((0.-(-100.))*result5[LSR]+(-100.-(-50.))*result4[LSR]+((-50.-(-25.))*(result3[LSR])+(-25.-(-70.))*(result2[LSR])-70.*(result1[LSR])));
	res[LSY] = (float)((0.-(-90.))*result5[LSY]+(-90.)*(result1[LSY]));
	res[LEB] = (float)((0.-(-10.))*result5[LEB]+(-10.-70.)*(result3[LEB])+(70.-52.)*(result2[LEB])+52.*(result1[LEB]));


			///End of the user modified code ////////////////////////////////////////////

			UpperMovement[RSP] = res[RSP];// R-Shoulder Pitch	
			UpperMovement[RSR] = res[RSR];// R-Shoulder Roll	
			UpperMovement[RSY] = res[RSY];// R-Shoulder Yaw
			UpperMovement[REB] = res[REB];// R-Elbow	Pitch
			
			UpperMovement[LSP] = res[LSP];// L-Shoulder Pitch
			UpperMovement[LSR] = res[LSR];// L-Shoulder Roll
			UpperMovement[LSY] = res[LSY];// L-Shoulder Yaw
			UpperMovement[LEB] = res[LEB];// L-Elbow	Pitch		
			
			UpperMovement[WST] = (float)0.;// Trunk
			
			UpperMovement[RWY] = res[RWY];
			UpperMovement[RW1] = (float)0.;
			UpperMovement[RW2] = (float)0.;
			
			UpperMovement[LWY] = res[LWY]; 
			UpperMovement[LW1] = (float)0.;
			UpperMovement[LW2] = (float)0.;
			...
			MotionHipAngPos[0] = (float)0.;// Hip Pitch
			MotionHipAngPos[1] = MotionHipAngPos[0];
			//////////
			for(i=RSP;i<LEB;i++)
			{
...
void Motion_Hello_World(int T, char MotionStop,unsigned int MotionNo)
{
	...
	if(MotionStop == 0)
	{
		...
		else if((time>=0)&&(time<=T))
		{
			...
			// user modified code /////////////////////////////////////////////////////////
			// In American Sign Language, this motion will sign "Hello world."
			...
			// Raise arm to "salute"
			// Angles
			// right shoulder pitch  angle 130
			// right shoulder yaw	 angle 50
			// right shoulder roll	 angle 10
			// right elbow bend		 angle 75

			// RSP
			FTN_half_1_cos( 1.0f,time,    50,100,0,0,&result1[RSP]);			
			result1[RSP] = (float)((130.) * result1[RSP]);
			
			// RSY
			FTN_half_1_cos( 1.0f,time,    50,100,0,0,&result1[RSY]);
			result1[RSY] = (float)((50.) * result1[RSY]);

			// RSR
			FTN_half_1_cos( 1.0f,time,    50,100,0,0,&result1[RSR]);
			result1[RSR] = (float)((10.) * result1[RSR]);

			// REB
			FTN_half_1_cos( 1.0f,time,    50,100,0,0,&result1[REB]);
			result1[REB] = (float)((75.) * result1[REB]);

			// Extend hand from forehead
			// right shoulder pitch  angle 110
			// right elbow bend		 angle 45

			// RSP
			FTN_half_1_cos( -1.0f,time,    150,100,0,0,&result2[RSP]);			
			result2[RSP] = (float)(((110.-130.) * result2[RSP] + 130.) + result1[RSP]);

			// REB
			FTN_half_1_cos( -1.0f,time,    150,100,0,0,&result2[REB]);			
			result2[REB] = (float)(((45.-75.) * result2[REB] + 75.) + result1[REB]);
